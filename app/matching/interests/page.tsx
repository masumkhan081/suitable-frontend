...
import { Heart, Users, Sparkles, ArrowLeft, Send, Clock, UserCheck, Inbox } from 'lucide-react'
...
export default function InterestsPage() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<string>(() => {
    ...
    if (typeof window !== 'undefined') {
      const savedSubTab = localStorage.getItem('last-active-interests-tab')
      return savedSubTab || 'received'
    }
    return 'received'
  })
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('last-active-interests-tab', activeSubTab)
    }
  }, [activeSubTab])
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-6">
        <div className="flex gap-6 min-h-[calc(100vh-200px)]">
          ...
          <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="p-8">
              <InterestsTabContent activeSubTab={activeSubTab} />
            </div>
          </div>
        </div>
      </div>
...
// Helper function for interests tabs
function getInterestsTabs() {
  return [
    { id: 'received', label: 'Received Interests', icon: Inbox },
    { id: 'sent', label: 'Sent Interests', icon: Send },
    { id: 'pending', label: 'Pending Response', icon: Clock }
  ]
}

// Interests Tab Content Component
function InterestsTabContent({ activeSubTab }: { activeSubTab: string }) {
  const getContentTitle = () => {
    const subTabData = getInterestsTabs().find(tab => tab.id === activeSubTab)
    return subTabData?.label || 'Interests'
  }

  const getContentDescription = () => {
    const descriptions: Record<string, string> = {
      received: 'People who have shown interest in your profile',
      sent: 'Interests you have sent to other profiles',
      pending: 'Interests waiting for your response'
    }
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
      ...
      <div className="space-y-4">
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            This section will display {activeSubTab} interests content
          </p>
        </div>
      </div>
    </div>
